...
class CodableFeedStoreTests: XCTestCase {
    ...
    func test_retrieve_deliversFailureOnRetrievalError() {
        ...
        try! "invalid cache".write(to: storeURL, atomically: false, encoding: .utf8)
    ...
    func test_insert_ovveridesPreviouslyInsertedCacheValues() {
        ...
        XCTAssertNil(latestInsertionError, "Expected to insert cache sucessfully")
    ...
    private func insert(_ cache: (feed: [LocalFeedImage], timestamp:Date), to sut: CodableFeedStore) -> Error? {
        ...
        sut.insert(cache.feed, cache.timestamp) { receivedError in
            inserationError = receivedError
            exp.fulfill()
        }
